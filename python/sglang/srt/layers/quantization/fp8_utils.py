...
def get_fp8_gemm_runner_backend() -> Fp8GemmRunnerBackend:
    ...
    if FP8_GEMM_RUNNER_BACKEND is None:
        backend = os.getenv("SGLANG_FP8_GEMM_BACKEND", "auto").strip().lower()
        try:
            FP8_GEMM_RUNNER_BACKEND = Fp8GemmRunnerBackend(backend)
        except ValueError as err:
            choices = ", ".join(item.value for item in Fp8GemmRunnerBackend)
            raise ValueError(
                "Unsupported SGLANG_FP8_GEMM_BACKEND="
                f"{backend!r}; expected one of: {choices}"
            ) from err
