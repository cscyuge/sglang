...
from sgl_kernel import rmsnorm, turbomind_rms_norm, turbomind_rms_norm_v0
...
def calculate_diff(token_num, head_num, head_dim):
    ...
    output_sgl_kernel = rmsnorm_sgl_kernel(
        x.reshape(-1, head_dim).clone(), weight
    )
    output_turbomind_sgl_kernel = turbomind_rmsnorm_sgl_kernel(
        x.clone(), weight, token_num=token_num, head_num=head_num, head_dim=head_dim, stride=x.shape[1]
    )

    output_sgl_kernel = output_sgl_kernel.reshape(token_num, head_num, head_dim)
    output_turbomind_sgl_kernel = output_turbomind_sgl_kernel.reshape(token_num, head_num, head_dim)
    ...
    if torch.allclose(
        ...
        ref_diff = torch.abs(output_sgl_kernel - output_turbomind_sgl_kernel)/torch.abs(output_sgl_kernel)
...
def get_benchmark():
    @triton.testing.perf_report(
        triton.testing.Benchmark(
            ...
            line_arg="provider",
            line_vals=["rmsnorm", "turbomind_rmsnorm"],
            line_names=["RMSNorm", "Turbomind RMSNorm"],
            styles=[("blue", "-"), ("green", "-")],
            ylabel="us",
            plot_name=f"rmsnorm-performance",
            args={},
        )
    )
...
if __name__ == "__main__":
    ...
    calculate_diff(
        token_num=1024, head_num=16, head_dim=128
    )
